...
TaskTray has an automated unit test suite covering the app's business logic:
...
TaskTray's actual invariants live: column ownership rules, the filed/unfiled
...
`App.tsx`) and the `useSortableItem` dnd-kit hook. TaskTray's own
